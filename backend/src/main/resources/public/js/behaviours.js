...
/***/ (function(module, exports, __webpack_require__) {
	...
	entcore_1.Behaviours.register('actualites', {
	    ...
	    resource: function (resource) {
	        if (resource !== undefined) {
	            ...
	            var hasAdmlRightsFor = function (behaviour, structureId) {
	                // Adml have rights on these behaviours
	                return ['editThread', 'deleteThread', 'manager', 'share'].indexOf(behaviour) >= 0 &&
	                    entcore_1.model.me.functions &&
	                    entcore_1.model.me.functions.ADMIN_LOCAL &&
	                    entcore_1.model.me.functions.ADMIN_LOCAL.scope &&
	                    entcore_1.model.me.functions.ADMIN_LOCAL.scope.indexOf(structureId) >= 0;
	            };
	            for (var behaviour in actualitesBehaviours.resources) {
	                if (entcore_1.model.me.hasRight(resource, actualitesBehaviours.resources[behaviour])
	                    || entcore_1.model.me.userId === resource.owner
	            ...
	            if (resource.shared && resource.shared.length > 0) {
	                resource.shared.forEach(function (shareInfos) {
	                    for (var behaviour in actualitesBehaviours.resources) {
	                        for (var share in shareInfos) {
	                            if (share.includes(behaviour) && entcore_1.model.me.userId === shareInfos.userId && shareInfos[share]) {
	                                if (resource.myRights[behaviour] !== undefined) {
	                                    resource.myRights[behaviour] = resource.myRights[behaviour] && actualitesBehaviours.resources[behaviour];
	                                }
	                                else {
	    ...
	    workflow: function () {
	        ...
	        for (var prop in actualitesWorkflow) {
	            if (entcore_1.model && entcore_1.model.me.hasWorkflow(actualitesWorkflow[prop])) {
	                workflow[prop] = true;
	            }
	        }
	        return workflow;

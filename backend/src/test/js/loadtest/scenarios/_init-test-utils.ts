...
const nbInfosPublish = __ENV.LOCAL_NB_INFOS_PUBLISH
  ? Number(__ENV.LOCAL_NB_INFOS_PUBLISH)
  : 300;
const nbInfosPending = __ENV.LOCAL_NB_INFOS_PENDING
  ? Number(__ENV.LOCAL_NB_INFOS_PENDING)
  : 30;
const nbInfosDraft = __ENV.LOCAL_NB_INFOS_DRAFT
  ? Number(__ENV.LOCAL_NB_INFOS_DRAFT)
  : 20;
...
const teachersRecords = new SharedArray("teachers", () =>
  parseCsv("../data/teachers.csv"),
);
const studentsRecords = new SharedArray("students", () =>
  parseCsv("../data/students.csv"),
);
const relativesRecords = new SharedArray("relatives", () =>
  parseCsv("../data/relatives.csv"),
);
const personnelsRecords = new SharedArray("personnels", () =>
  parseCsv("../data/personnels.csv"),
);
...
export function initLocal(schoolName: string): InitData {
  ...
  describe("[Info-Widget-Init] Initialize data", () => {
    ...
    const thread: ThreadIdentifier = createThreadOrFail(
      `Thread 1 ${seed}`,
      head.id,
    );
    ...
    shareThreadOrFail(
      thread.id,
      [
        teacherProfileGroup.id,
        personnelProfileGroup.id,
        studentProfileRole.id,
        relativeProfileRole.id,
      ],
      threadAllRights,
      ShareTargetType.GROUP,
    );

    const targetHeadUsers = headUsers.filter(
      (headUser) =>
        headUser.type === "Teacher" || headUser.type === "Personnel",
    );
    const targetSchoolUsers = schoolUsers.filter(
      (headUser) => headUser.type === "Student" || headUser.type === "Relative",
    );
...
export function initFromCsv(): InitData {
  ...
  describe("[Info-Widget-Init] Initialize from CSV files ", async () => {
    ...
    console.log(
      `Authenticating ${userRecords.length} users in batches of ${authBatchSize}...`,
    );
    ...
    for (
      let batchStart = 0;
      batchStart < userRecords.length;
      batchStart += authBatchSize
    ) {
      ...
      console.log(
        `Batch ${Math.floor(batchStart / authBatchSize) + 1}: authenticated ${batchEnd}/${userRecords.length} users`,
      );
    }

    console.log(
      `Successfully authenticated ${successCount}/${userRecords.length} users`,
    );
  });
  initValidator(initData);
  return initData;
...
function initValidator(data: InitData) {
  const users = data.allSessions.filter(
    (user: InfoUser) => user.role === "PUBLISHER",
  );
  const user = users[0];
  ...
  const totalVu = Math.floor(
    ((parseDuration(duration) / 1000) * pendingVus) / 20,
  );
